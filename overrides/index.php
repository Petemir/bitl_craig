...
if (isset($_REQUEST["delete"])) {
    ...
    header("Content-type: application/json");
    print "{\"duration\":";
    passthru("/bitl_craig/cook/duration.sh $id");
    print "}";
    ...
    flush();
    set_time_limit(300);
    passthru("/usr/bin/timeout 7200 /bitl_craig/cook.sh $id $format $container$exflags");
    ...
    ob_flush();
    flush();
    passthru("/usr/bin/timeout 14400 /bitl_craig/cook/avatars.sh $id $format $container $transparent $bg $fg");

} else if (isset($_REQUEST["fetch"]) && $_REQUEST["fetch"] == "info") {
    header("Content-type: application/json");
    passthru("/bitl_craig/cook/info.sh $id");

} else if (isset($_REQUEST["fetch"]) && $_REQUEST["fetch"] == "infotxt") {
    header("Content-disposition: attachment; filename=$id-info.txt");
    header("Content-type: text/plain");
    passthru("/bitl_craig/cook/infotxt.sh $id");
